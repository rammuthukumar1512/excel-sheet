import type { Cell, Editor, Styles } from "../types/cell";
...
export class SpreadSheet extends HTMLElement {
    ...
    styleText(selRange:any,editor: any, completeEditor: Editor, editorText:string) {
         let html = "";
         let subStyleText = "";
         ...
         completeEditor.forEach((value, index)=> {
            ...
            if(value.subStyles.length == 1) {
               value.subStyles.forEach((styleItem, index)=> {
                ...
                console.log(subStyles,"styles")
                  console.log("0")
                  if(value.startOffset !== styleItem.startOffset) { 
                        subStyleText = `<span style="${style}">${editorText.substring(value.startOffset, styleItem.startOffset)}</span>` 
                        subStyleText += `<span style="${subStyles}">${editorText.substring(styleItem.startOffset, styleItem.endOffset)}</span>`;
                  } else subStyleText += `<span style="${subStyles}">${editorText.substring(styleItem.startOffset, styleItem.endOffset)}</span>`;      
                  if(styleItem.endOffset < value.endOffset) subStyleText += `<span style="${style}">${editorText.substring(styleItem.endOffset, value.endOffset)}</span>`;    
                  console.log(subStyleText,"cvc")
                });
            } 
            else if(value.subStyles.length > 1) {
            ...
            value.subStyles.forEach((styleItem, index)=> {
                ...
                if(selRange.start == value.startOffset && selRange.end == value.endOffset) {
                    let styleOne = subStyles.split(";");
                    let styleTwo = style.split(";");
                    let filteredStyle = styleTwo.filter(style => !styleOne.includes(style)).join(";");
                    console.log(styleOne,"styleone",filteredStyle)
                    // subStyles += filteredStyle;
                    console.log(subStyles,"styles2")
                }
                ...
                if(index == 0) {
                  ...
                  console.log(subStyleText,"ste")
                } else if(index == value.subStyles.length - 1) {
                  console.log("3")
                  subStyleText += `<span style="${subStyles}">${editorText.substring(styleItem.startOffset, styleItem.endOffset)}</span>`;
                  if(styleItem.endOffset < value.endOffset) subStyleText += `<span style="${style}">${editorText.substring(styleItem.endOffset, value.endOffset)}</span>`;
                  console.log(subStyleText,"stext")
                }
                else {
                  console.log("2")
                  subStyleText += `<span style="${subStyles}">${editorText.substring(styleItem.startOffset, styleItem.endOffset)}</span>`;
                  console.log(subStyleText,"slast")
                }
                subStyles = "";
                console.log(subStyleText,"subtext")
            });
            } else {
                subStyleText = `<span style="${style}">${editorText.substring(value.startOffset, value.endOffset)}</span>`;
                console.log(subStyleText,"ste")
            }
    ...
    checkAllStylesEmpty(completeEditor:Editor) {
        return completeEditor.every((modal:any)=> {
          console.log(modal.styles,"34")
          return !Object.keys(modal.styles).length;
        });
    }
    ...
    checkAllSubstylesIsEmptyWhenAllStylesIsEmpty(completeEditor:Editor) { return completeEditor.every(modal => !modal.subStyles.length);}
    ...
    findSubStyles(selRange:any, completeEditor:Editor, styleKey: string, styleValue: string) {
          ...
          console.log(modal,"check123456789")
           if(modal.subStyles.length) {
              ...
              modal.subStyles.forEach((value:any, index:number) =>{
                ...
                //  styles.push({styles: {...modal.styles, ...value.styles}, startOffset: modal.startOffset, endOffset: value.endOffset});
                // } else if(index == modal.subStyles.length - 1){
                //  styles.push({styles: {...modal.styles, ...value.styles}, startOffset: value.startOffset, endOffset: value.endOffset}); 
                //  styles.push({styles: {...modal.styles}, startOffset: value.endOffset, endOffset: modal.endOffset}); 
                // } else {
                //  styles.push({styles: {...value.styles}, startOffset: value.startOffset, endOffset: value.endOffset}); 
                // }
              });
              ...
              if(modal.subStyles.length) {
              let existSubstyles = modal.subStyles.filter((value:any, index:number) =>{
                  return selRange.start >= value.startOffset && selRange.end >= value.endOffset;
              });
              modal.subStyles.forEach((subst:any, i:number)=>{
                  if(selRange.start == subst.startOffset && selRange.end == subst.endOffset) {
                    console.log(subst,"subtmuthu")
                   styles.push({styles: {...subst.styles}, startOffset: subst.startOffset, endOffset: subst.endOffset});
                  }
              });
           }
        } else if(selRange.start <= modal.startOffset && selRange.end <= modal.endOffset) {
      ...
      if(this.checkAllSubStylesEmpty(styles)) {
        styles = [];
      }
    ...
    findChildModalsFromSubstyles(range:any, substyles:Styles) {
       return substyles.filter((val:any, index: number)=> range.startOffset <= val.startOffset && range.endOffset >= val.endOffset);
    };

    findSubStylesWithoutSelection(range: any, substyles:Styles) {
      return substyles.filter((val:any, index: number)=> {
        if((range.startOffset <= val.startOffset && range.endOffset <= val.endOffset && val.startOffset < range.endOffset)) {
          val.endOffset = range.endOffset;
        }
      return (range.startOffset <= val.startOffset && range.endOffset >= val.endOffset) || (range.startOffset <= val.startOffset && range.endOffset <= val.endOffset && val.startOffset < range.endOffset)
      }
      );
    };
    ...
    applyStyleToSelection(styleKey: string, styleValue: string):void {
      ...
      const editorText = editor.innerHTML.toString().replace(/<\/?span[^>]*>/g, "");
      console.log(editorText,"txtedit")
      textAndStyles= [{styles: {}, startOffset: 0, endOffset: editorText.length}];
      let completeEditor: Editor = this.grid[this.selectedCell.row][this.selectedCell.col].editor;
      let sampleStyles = structuredClone(completeEditor);
      let style = {styles: {}, startOffset: selRange.start, endOffset: selRange.end};
      sampleStyles.forEach((value, index) => {
            if(selRange.start > value.startOffset) {
              textAndStyles.splice(index + 1, 0, style);
            }
      });
      ...
      if(completeEditor.length == 1) {
         if(selRange.start !== 0) {
           completeEditor[0] = {text:editorText.substring(initialRange, endRange), styles: {}, subStyles:[],startOffset: 0, endOffset: selRange.start};
           completeEditor.push({text:editorText.substring(selRange.start, selRange.end), styles: {[styleKey]: styleValue}, subStyles:[], startOffset: selRange.start, endOffset: selRange.end});
           editorText.substring(selRange.end).length ? completeEditor.push({text:editorText.substring(selRange.end), styles: {}, subStyles:[], startOffset: selRange.end, endOffset: editorText.length}) : "";
           this.grid[this.selectedCell.row][this.selectedCell.col].editor = [...completeEditor];
         } else {
           completeEditor[0] = {text:editorText.substring(selRange.start, selRange.end), styles: {[styleKey]: styleValue}, subStyles:[], startOffset: selRange.start, endOffset: selRange.end};
           if(selRange.end !== editorText.length) completeEditor.push({text:editorText.substring(selRange.end, editorText.length), styles: {},subStyles:[], startOffset: selRange.end, endOffset: editorText.length});
         }
         console.log(completeEditor,"fe");
         this.styleText(selRange,editor, completeEditor, editorText);
         return;
      } 
      if(completeEditor.length > 1) {
         ...
         if(completeEditor[completeEditor.length - 1].endOffset < editorText.length){
            completeEditor[completeEditor.length] = {text:editorText.substring(completeEditor[completeEditor.length - 1].endOffset, editorText.length), styles: {[styleKey]: styleValue},subStyles:[{styles: {}, startOffset: 0, endOffset: editorText.length}], startOffset: completeEditor[completeEditor.length - 1].endOffset, endOffset: editorText.length};
         };
         ...
         tempEditor.forEach((modal: any, index: number)=>{
             ...
             if(selRange.start == modal.startOffset && selRange.end < modal.endOffset) {
                  ...
                  if(modal.subStyles.length) {
                  let range = {startOffset: selRange.end, endOffset: modal.endOffset};
                  childSubstyles = this.findChildModalsFromSubstyles(range, modal.subStyles);
                  console.log(childSubstyles,"dcf")
                  }
                  if(childSubstyles.length) modal.styles = childSubstyles[0].styles;
                  completeEditor.splice(index + 1, 0, {text:editorText.substring(selRange.end, modal.endOffset), styles: {...modal.styles},subStyles:[...childSubstyles], startOffset: selRange.end , endOffset: modal.endOffset});
                  if(this.checkAllStylesEmpty(completeEditor) && this.checkAllSubstylesIsEmptyWhenAllStylesIsEmpty(completeEditor)) {
                    completeEditor = [{text: editorText.substring(0, editorText.length),styles: {},subStyles:[],startOffset: 0, endOffset: editorText.length}];
                  }
                  this.grid[this.selectedCell.row][this.selectedCell.col].editor = [...completeEditor];
                  console.log(completeEditor,"ab12");
                  isStyleApplied = true;
                  this.styleText(selRange,editor, completeEditor, editorText);
                  return;
             }
             ...
             if(selRange.start == modal.startOffset) {
                  ...
                  // if(completeEditor[index].endOffset == completeEditor[index + 1].endOffset) {
                  //   completeEditor.splice(index + 1, 1);
                  // }
                  console.log(completeEditor, "coml123")
                  ...
                  newEditor.forEach((modal: any, lastModalIndex: number) => {
                    ...
                    if(modal.startOffset === selRange.start) {
                      ...
                      if(!!completeEditor[index + 1]) {
                        console.log("jj")
                        let subStylesOfNextModal = structuredClone(completeEditor[index + 1].subStyles);
                        const subStylesAfterSelection = this.findSubStylesAfterSelection({startOffset: selRange.end, endOffset: completeEditor[index + 1].endOffset}, subStylesOfNextModal, styleKey, styleValue);
                        completeEditor.splice(lastModalIndex + 1, 1, {text:editorText.substring(selRange.end, completeEditor[index + 1].endOffset), styles: {...completeEditor[index + 1].styles},subStyles:[...subStylesAfterSelection], startOffset: selRange.end , endOffset: completeEditor[index + 1].endOffset});
                        // if(completeEditor[lastModalIndex].endOffset !== completeEditor[lastModalIndex + 1]?.startOffset) completeEditor.splice(lastModalIndex + 1, 1, {text:editorText.substring(selRange.end, completeEditor[lastModalIndex + 1]?.endOffset), styles: {styleKey: styleValue}, startOffset: selRange.end , endOffset: completeEditor[lastModalIndex + 1]?.endOffset});
                        }
                      if(this.checkAllStylesEmpty(completeEditor) && this.checkAllSubstylesIsEmptyWhenAllStylesIsEmpty(completeEditor)) {
                        completeEditor = [{text: editorText.substring(0, editorText.length),styles: {},subStyles:[],startOffset: 0, endOffset: editorText.length}];
                      }
    ...
    getSelectionRange() {
        const selection = window.getSelection();
        console.log(selection?.getRangeAt(0),"sawsel")
